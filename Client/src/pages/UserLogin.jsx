  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
          ...
          login(token); // Use context's login method
          navigate('/home');
      }
  } catch (error) {
    ...
    <div className='p-7 h-screen flex flex-col justify-between'>
      <div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button className='bg-[#111] text-white font-semibold mb-7 rounded px-2 py-2 w-full text-lg'>Login</button>
        </form>
      ...
      <div> 
        <Link to='/captain-login' className='bg-[#10b461] text-white flex items-center justify-center font-semibold mb-5 rounded px-2 py-2 w-full text-lg'>Sign In as Captain</Link>
      </div>
    </div>
  )
}
